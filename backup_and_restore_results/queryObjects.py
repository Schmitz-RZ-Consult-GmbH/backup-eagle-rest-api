#######################################################################################################
#
# Python Example - Query backup/restore objects for a backup/restore result
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage:
# python queryObjects.py --host="192.168.0.1" --user="admin" --password="admin" --pretty-print=True --ba-result-id="57091"
#
...
def validate_input():
    if(options.username is None or options.password is None or options.host is None or options.baResultId is None):
...
def main():
    ...
    url = "https://" + options.host + ":10987/api/rest/baobject/get?baResultId=" + options.baResultId
