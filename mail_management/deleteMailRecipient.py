#######################################################################################################
#
# Python Example - Delete mail recipients
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage:
# python deleteMailRecipient.py --host="192.168.0.1" --user="admin" --password="admin" --recipient-id="2"
#
...
parser.add_option("--recipient-id", dest="recipientId", help="Id of the mail recipient.")
parser.add_option("--verify-ssl", dest="verifySSL", default=False, help="Yerify SSL certificate of connection (default: false)")

(options, args) = parser.parse_args()

def validate_input():
    if(options.username is None or options.password is None or options.host is None or options.recipientId is None):
...
def main():
    ...
    url = "https://" + options.host + ":10987/api/rest/mailrecipients/delete?id=" + options.recipientId
