#######################################################################################################
#
# Python Example - Update newsgroup
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage:
# python updateNewsGroup.py --host="192.168.0.1" --user="admin" --password="admin" --group='{\"name\":\"All_PDFs\",\"pattern\":\"*.pdf\",\"subject\":\"BACKUP_EAGLE_Reports\",\"text\":\"BACKUP_EAGLE_Reports\"}'
#
...
parser.add_option("--group-id", dest="groupId", help="Id of the newsgroup")
parser.add_option("--group", dest="group", help="Newsgroup (name*, pattern*, subject*, text*, activeFrom, activeTo, mergeMails, recipientIds, recipientGroupIds) as json. (* required)")
parser.add_option("--verify-ssl", dest="verifySSL", default=False, help="Yerify SSL certificate of connection (default: false)")

(options, args) = parser.parse_args()

def validate_input():
    if(options.username is None or options.password is None or options.host is None or options.groupId is None or options.group is None):
...
def main():
    ...
    url = "https://" + options.host + ":10987/api/rest/mail/newsgroup/" + options.groupId
