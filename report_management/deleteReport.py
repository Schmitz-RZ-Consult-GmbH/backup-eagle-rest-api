#######################################################################################################
#
# Python Example - Delete report
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage:
# python deleteReport.py --host="192.168.0.1" --user="admin" --password="admin" --report-id="7"
#
...
parser.add_option("--report-id", dest="reportId", help="Id of the report.")
parser.add_option("--verify-ssl", dest="verifySSL", default=False, help="Yerify SSL certificate of connection (default: false)")

(options, args) = parser.parse_args()

def validate_input():
    if(options.username is None or options.password is None or options.host is None or options.reportId is None):
...
def main():
    ...
    url = "https://" + options.host + ":10987/api/rest/reports/delete?id=" + options.reportId
