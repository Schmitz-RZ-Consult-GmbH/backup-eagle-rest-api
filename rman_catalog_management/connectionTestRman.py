#######################################################################################################
#
# Python Example - Test the RMAN catalog config to the RMAN database
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage: 
# python connectionTestRman.py --host="192.168.0.1" --user="admin" --password="admin" --config-id="170018409"
#
...
def validate_input():
    if(options.username is None or options.password is None or options.host is None or options.configId is None):
...
def main():
    ...
    url = "https://" + options.host + ":10987/api/rest/config/rman/" + options.configId + "/connectiontest"
