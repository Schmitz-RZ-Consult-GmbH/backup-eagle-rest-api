#######################################################################################################
#
# Python Example - Create a new RMAN catalog config
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage:
# python createRmanCatalogConfig.py --host="192.168.0.1" --user="admin" --password="admin" --config='{\"user\":\"user\",\"password\":\"password\",\"connectionString\":\"jdbc:oracle:thin:@192.168.111.100:1521:orcl\",\"active\":true,\"credentialsRequired\":true,\"scheme\":\"\"}'
#
...
parser.add_option("--config", dest="config", help="User, password, connectionString (required), active (required), credentialsRequired and scheme as json.")
parser.add_option("--verify-ssl", dest="verifySSL", default=False, help="Yerify SSL certificate of connection (default: false)")

(options, args) = parser.parse_args()

def validate_input():
    if(options.username is None or options.password is None or options.host is None or options.config is None):
...
def main():
    ...
    url = "https://" + options.host + ":10987/api/rest/config/rman"
