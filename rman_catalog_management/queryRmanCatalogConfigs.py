#######################################################################################################
#
# Python Example - Query RMAN catalog config
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage:
# python queryRmanCatalogConfigs.py --host="192.168.0.1" --user="admin" --password="admin" --pretty-print=True
#
...
parser.add_option("--catalog-id", dest="catalogId", help="Query catalog config with specific id (optional)")
...
def main():
    ...
    if(options.catalogId is not None):
        url += "/" + options.catalogId
