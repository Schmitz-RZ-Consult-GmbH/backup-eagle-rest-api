#######################################################################################################
#
# Python Example - Create a role
# (C) Schmitz RZ Consult GmbH - BACKUP EAGLE
#
# Example usage:
# python createRole.py --host="192.168.0.1" --user="admin" --password="admin" --role='{\"roleId\":22,\"userDefGroupId\":1}'
#
...
parser.add_option("--role", dest="role", help="RoleId (required), userDefGroupId, serverGroupId and deviceGroupId as json.")
parser.add_option("--verify-ssl", dest="verifySSL", default=False, help="Yerify SSL certificate of connection (default: false)")

(options, args) = parser.parse_args()

def validate_input():
    if(options.username is None or options.password is None or options.host is None or options.role is None):
...
def main():
    ...
    url = "https://" + options.host + ":10987/api/rest/security/role"
    data = json.loads(options.role)
